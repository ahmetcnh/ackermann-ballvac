...
- **Cameras**:
  ...
  - Left-side
  - Right-side
  > **Note:** By default, only the front camera is bridged to ROS 2.If you want to use all cameras (left, right, rear) in ROS 2,remove the `#` at the beginning of the relevant camera sections in `ballvac_bringup/config/ros_gz_bridge.yaml` to activate them  (e.g., `/camera/left_raw`, `/camera/right_raw`, `/camera/rear_raw`).
...
1.  Launch the simulation:
    ```bash
    ros2 launch ballvac_bringup ballvac_spawn.launch.py
    ```
2.  Control car:
    ```bash
...
| ![alt text](ballvac_msgs/readme_files/ballvac.png)                                                                                         | ![alt text](ballvac_msgs/readme_files/rviz_ballvac.png)                                                                                    |

## TF Tree

![TF Tree](ballvac_msgs/readme_files/frames.png)
